...
Swirl allows you to pass [comptime expressions](comptime.md) as generic arguments, to do so,
...
```
## Generic Constraints
Generics parameters can be constrained by one or multiple [protocols](protocols.md):
```swirl
...
both cases to implement all the prescribed [protocols](protocols.md).
